...
def process_svg(inpath):
	...
	# this can also be in mm (millimeters).
	# if the string ends in "mm", we divide by the correct fractions to get LaTeX points
	width = doc.getroot().attrib['width']
	
	if width.endswith('mm'):
		width = MM_TO_BIG_POINTS*(float(width.split('mm')[0]))
	else:
		width = float(width) * SVG_UNITS_TO_BIG_POINTS 
	
	height = doc.getroot().attrib['height']
	if height.endswith('mm'):
		height = MM_TO_BIG_POINTS*(float(height.split('mm')[0]))
	else:
		height = float(height) * SVG_UNITS_TO_BIG_POINTS
